...
const outputDir = join(repoRoot, "switch-port", "stages");
...
function buildStageExport(stage, index) {
  const bgmTrack = BGM_TRACKS[stage.bgm.track];
  if (!bgmTrack) {
    throw new Error(`Unknown BGM track for ${stage.id}: ${stage.bgm.track}`);
  }
  ...
  return {
    schemaVersion: 1,
    gameId: GAME_ID,
    exportId: `switch-stage${index + 1}-${stage.id}`,
    ...
    scenario: {
      ...
      finalRevealLines: stage.finalRevealLines ?? [],
    },
    enemy: stage.enemy,
    audio: {
      bgm: {
...
function stageFileName(stage, index) {
  return `stage${String(index + 1).padStart(2, "0")}-${stage.id}.stage.json`;
}

mkdirSync(outputDir, { recursive: true });

for (const [index, stage] of STAGES.entries()) {
  const outputPath = join(outputDir, stageFileName(stage, index));
  const payload = buildStageExport(stage, index);
