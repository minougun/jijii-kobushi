...
function assertChart(difficulty, chart, sourceChart) {
  ...
  const typeCounts = chart.reduce((acc, note, index) => {
    ...
    const expectedNote = normalizeSourceNote(sourceChart[index], difficulty, index);
    assert.deepEqual(note, expectedNote, `${difficulty}:${index}: note payload must match src/stages.js`);
    acc[note.type] = (acc[note.type] ?? 0) + 1;
    return acc;
  }, {});

  assert.deepEqual(typeCounts, summary.typeCounts, `${difficulty}: type counts`);
}

const filePath = process.argv[2];
if (!filePath) {
  console.error("Usage: node scripts/validate-switch-stage1-export.mjs <stage-json>");
  process.exit(2);
}

const payload = readJson(filePath);
const stage = STAGES[EXPECTED_STAGE_INDEX];
assert.ok(stage, "missing Stage 1 source stage");
assert.equal(stage.id, STAGE_ID, "Stage 1 source id");
