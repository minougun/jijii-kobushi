...
for (const stage of STAGES) {
  ...
  if (!existsSync(filePath)) bgmErrors.push(`${stage.id}: missing bgm file ${filePath}`);
}
...
for (const fileName of REQUIRED_IMAGE_FILES) {
  ...
  if (!existsSync(filePath)) bgmErrors.push(`missing image file ${filePath}`);
}
for (const fileName of REQUIRED_FONT_FILES) {
  const filePath = join("assets", "fonts", fileName);
  if (!existsSync(filePath)) bgmErrors.push(`missing font file ${filePath}`);
}
