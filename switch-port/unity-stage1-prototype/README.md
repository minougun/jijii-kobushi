...
`StageBgmCredit` records the current stage-BGM provider line used by the Unity prototype. The temporary Play Mode scene draws `Music: PeriTune` at the lower-left of the stage screen, and EditMode coverage checks every exported stage pack has the same credit line. The source notes remain in the Web original audio README; before external release, confirm current provider license text from the official source.

`StageRuntimeVisualAssets` maps the seven portable stage ids to the Web original PNG background sources and fixes the current Web chibi character-sheet sprite contract for the player and enemy kinds. `PlaceholderRendererBehaviour` loads backgrounds, the chibi character sheet, the Web `kojiro-cutin.png` special cut-in, and the final reveal sprite through the same runtime asset resolver, so the Editor and built smoke player can draw staged `StreamingAssets` backgrounds/characters/cut-ins/reveal art when available while still falling back to local Web-original files during development. The character-sheet map keeps the Unity port aligned with the Web `renderer.js` sprite indices until the final renderer replaces the placeholder IMGUI layer. The temporary IMGUI battle scene now flashes the special cut-in during finisher-note windows, preserving the Web-side cue for ten-hit follow-ups and the post-dojo `爺コブシ` internal-destruction phase. The final-stage result panel also shows the Hasegawa reveal sprite before handing off to the ED bonus.
...
Character-facing temporary presentation lives in `PrototypeCharacterPanel`: chibi sprite-sheet slicing, finisher cut-in drawing, and the final reveal sprite are separated from the main scene driver so asset loading/state progression can keep shrinking independently from presentation.
...
`PlaceholderRendererBehaviour` now includes a temporary playable HUD: HP, score, combo, judge counts, current note, a page-advancing stage intro overlay, scenario preview, result panel, Prev/Next stage buttons, footer `Save`/`Load` buttons, result-panel `Next Stage` progression, final-stage `ED Bonus` handoff, and a simple rhythm lane that draws upcoming `TAP`, `HOLD`, and `MASH` notes against a gold hit line. The stage intro overlay holds BGM/count-in until the player advances all intro lines, matching the Web flow more closely than the previous one-line debug preview. Low-level IMGUI rectangle and meter drawing lives in `PrototypeGui`, reusable status panels live in `PrototypeStatusPanel`, footer/input controls live in `PrototypeFooterControls`, result-card pieces live in `PrototypeResultPanel`, and note-lane presentation lives in `PrototypeRhythmLane`, keeping the scene driver focused on game state and layout. `HOLD` notes include a yellow `RELEASE` marker at the note end so the button-up timing is visible.

The local result panel mirrors the Web original result-screen redesign: a large rank badge, score headline, stat tiles, and clearer stage/ED actions instead of raw debug text. It is still IMGUI placeholder rendering, but the information hierarchy now follows the commercial-style Web result card. Stage and ED result-card layout now lives in `PrototypeResultPanel`; the scene driver only passes state and callbacks.
