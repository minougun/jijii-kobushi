...
namespace JijiiKobushi.Stage1Prototype
{
    public static class RuntimeAssetImportPlanner
    {
        public static RuntimeAssetImportPlan Build(RuntimeAssetManifest manifest, string repoRoot, bool checkLocalFiles)
        {
            ...
            var normalizedRepoRoot = RuntimeAssetPathUtility.NormalizeRepoRoot(repoRoot);
            ...
            foreach (var asset in manifest.Assets)
            {
                ...
                if (checkLocalFiles && !File.Exists(Path.Combine(normalizedRepoRoot, RuntimeAssetPathUtility.NormalizeAssetPath(asset.Path).Replace('/', Path.DirectorySeparatorChar))))
                {
