...
The game uses 6 PeriTune MP3 core tracks selected for stronger enka/kayo fit across stages 1-6, plus 1 boss-only battle track for stage 7. Reused tracks play from the beginning at natural `1.0x`; stage variation comes from gain, WebAudio EQ tone, stronger stage-specific remix layers, and short original WebAudio kobushi-style voice callouts rather than different start offsets.
...
- Stage 3 `riverside`: `shizima4.mp3` from `Shizima4`, night tone, start `0s`
- Stage 4 `mountain`: `hanagoyomi2.mp3` from `Hanagoyomi2`, night tone, `toge` remix
- Stage 5 `garage`: `taishoroman-battle.mp3` from `TaishoRoman_Battle`, battle tone, `garage` remix
- Stage 6 `redgate`: `amenoshita3.mp3` from `Amenoshita3`, final tone, start `0s`
- Stage 7 `finalhideout`: `epicbattle-j.mp3` from `EpicBattle_J`, boss tone, `boss` remix
...
- The current build prioritizes consistency while avoiding an obvious Stage 1 / Stage 3 duplicate: `Koi_Wazurai`, `Oboro`, `Shizima4`, `Hanagoyomi2`, `TaishoRoman_Battle`, and `Amenoshita3` are the 6 enka/kayo core tracks.
- Stage 7 is an explicit exception: `EpicBattle_J` is used because the final boss should feel like a decisive battle even if it leaves the enka/kayo frame.
- Festival-only or scenic/action tracks such as `Harvest_Festival`, `RetroRoman_Battle3`, and `RetroRoman_City` are kept in the folder for provenance/testing but are not stage-assigned in the current build.
